...
////////////////////meggie
void VersionSet::PrintOverlappingRatio(Compaction* c) {
    assert(c->level() > 0);
    ...
    for(; i < sz2 * 2; i++) {
        ...
        if(icmp_.Compare(points[i], inputs1[j]->smallest) < 0) {
            ...
            continue;
        } else if(icmp_.Compare(points[i], inputs1[j]->largest) > 0 && j < (sz1 - 1)) {
            j++;
            delete iter;
            iter = table_cache_->NewIterator(ReadOptions(), 
                                inputs1[j]->number,
    ...
    for(i = 0; i < result.size(); i = i + 2) {
        ...
        DEBUG_T("sz:%lld, ro:%lf; ", inputs2[i/2]->file_size, ratio);
    }
    DEBUG_T("\n\n");
}
////////////////////meggie
